...
interface CartContextType {
  ...
  // allow callers to optionally specify quantity when bulk-adding
  addMultipleToCart: (
    products: (Omit<CartItem, "quantity"> & { quantity?: number })[]
  ) => void;
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addMultipleToCart = (
    ...
    setItems((currentItems) => {
      ...
      // Add each new product (respecting its requested quantity if provided)
      for (const product of products) {
        ...
        if (existingItem) {
          ...
          // Add new item
          updatedItems.push({ ...product, quantity: productQuantity });
        }
      }
